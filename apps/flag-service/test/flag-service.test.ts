import { createFlagApi } from '../src/api';
import { createTestDb, type db } from '../src/db';
import { testClient } from 'hono/testing';
import { beforeAll, beforeEach, describe, expect, test } from 'bun:test';
import { features, featureStates } from '../src/schema';

describe('Flag Service API', async () => {
  let testDb: db;
  let testApiClient = testClient(await createFlagApi());

  beforeAll(async () => {
    testDb = await createTestDb('flagservice');
    testApiClient = testClient(await createFlagApi(), { db: testDb });
  ...
  test('GET /flagd - Generate flagd config', async () => {
    ...
    const res = await testApiClient['flagd.json'].$get();
